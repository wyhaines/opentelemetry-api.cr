  ...
  module API
    abstract struct AbstractSpanContext
      abstract class AbstractConfig
        ...
        abstract def trace_id=(val : Slice(UInt8))

        # Likely defined as a property
        abstract def span_id : Slice(UInt8)
        abstract def span_id=(val : Slice(UInt8))

        # Likely defined as a property
        abstract def parent_id : Slice(UInt8)?
        abstract def parent_id=(val : Slice(UInt8)?)

        # Likely defined as a property
        abstract def trace_flags : TraceFlags
        abstract def trace_flags=(val : TraceFlags)

        # Likely defined as a property
        abstract def trace_state : Hash(String, String)
        abstract def trace_state=(val : Hash(String, String))

        # Likely defined as a property
        abstract def remote : Bool
        abstract def remote=(val : Bool)
      end
    end
  end
end
